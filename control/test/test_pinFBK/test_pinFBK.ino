...
boolean chequeoFBK (int PIN_MECANISMO_FBK){
    Serial.print("Se chequea en PIN  ");
    Serial.println(PIN_MECANISMO_FBK);
    if (digitalRead(PIN_MECANISMO_FBK)){
    // Si recibo el FBK correctamente determino FBK OK
        IMPRIMIR("Primer if chequeo FBK");
        FBK_OK;
        return true; 
    }
    else{
    // Si no recibo FBK correctamente espero un tiempo y vuelvo a intentar una vez más   
        delay(tiempoFBK);
        IMPRIMIR("delay en chequeo FBK");
        if(digitalRead(PIN_MECANISMO_FBK)){
            IMPRIMIR("Segundo if chequeo FBK");
            FBK_OK; 
            return true;
        }
        else{
...
void activarConEspera(int PIN_MECANISMO, int PIN_MECANISMO_FBK){
   boolean status;
   esperoOK = true;
   //Espero recibir "OK" para continuar al siguiente mecanismo
   while (esperoOK) {
    if (Serial.available() > 0) {
      String input = Serial.readString();
      if (input.equals("OK\n")) {
        esperoOK = false; // Cambia el estado para continuar
        Serial.flush(); //Reinicio el buffer, sino siempre toma como que ya se mandó un "OK"
        input = " ";
        }
      }
    }
    ...
    digitalWrite(PIN_MECANISMO, HIGH);
    delay(tiempoFBK);
    status = chequeoFBK(PIN_MECANISMO_FBK);
    Serial.print("Status = ");
    Serial.println(status);
    digitalWrite(PIN_MECANISMO,LOW); 
}

void pinFBK(){

  // Activa cada pin
  // Se fija si el feedback es correcto: avisa si no es así 
  
  IMPRIMIR("****** P I N   F B K *****");
  
//Flippers
  //Flipper IZQUIERDO
    IMPRIMIR("Activación FLIPPER IZQUIERDO");
    activarConEspera(FLIPPER_IZQUIERDO, FLIPPER_IZQUIERDO_FBK);
    
  //Flipper DERECHO
    IMPRIMIR("Activación FLIPPER DERECHO");
    activarConEspera(FLIPPER_DERECHO, FLIPPER_DERECHO_FBK);

//Slingshots
  //SLINGSHOT IZQUIERDO
    IMPRIMIR("Activación SLINGSHOT IZQUIERDO");
    activarConEspera(SLINGSHOT_IZQUIERDO, SLINGSHOT_IZQUIERDO_FBK);
    
  //SLINGSHOT DERECHO
    IMPRIMIR("Activación SLINGSHOT DERECHO");
    activarConEspera(SLINGSHOT_DERECHO, SLINGSHOT_DERECHO_FBK);

//Bumpers  
  //BUMPER IZQUIERDO
    IMPRIMIR("Activación BUMPER IZQUIERDO");
    activarConEspera(BUMPER_IZQUIERDO, BUMPER_IZQUIERDO_FBK);
    
  //BUMPER DERECHO
    IMPRIMIR("Activación BUMPER DERECHO");
    activarConEspera(BUMPER_DERECHO, BUMPER_DERECHO_FBK);
    
//Ball return
    IMPRIMIR("Activación BALL RETURN");
    activarConEspera(BUMPER_DERECHO, BUMPER_DERECHO_FBK);
}
